...
export default function Settings() {
  ...
  // Sidebar items (yahan aur items add kar sakte ho)
  const items = [
    { label: "Edit profile", href: "/UserEditProfile" },
    { label: "Notifications", href: "/Notification" },
    { label: "Account privacy", href: "/AccountPrivacy" },
  ];
    ...
    <div className="w-[24rem] bg-black border-r border-gray-800 min-h-screen">
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="space-y-2">
          ...
          {items.map((item) => {
            const active = pathname?.startsWith(item.href); // 👈 active check
            return (
              <button
                key={item.href}
                ...
                className={[
                  ...
                  active
                    ? "bg-[#262626] text-white" // 👈 active styles
                    : "hover:bg-[#262626] text-gray-100", // inactive/hover
                ].join(" ")}
              >
                <span className="text-sm">{item.label}</span>
                <ChevronRight
                  size={18}
                  className={active ? "text-gray-300" : "text-gray-500 group-hover:text-gray-400"}
                />
              </button>
            );
          })}
